...
Completes the scenario loop: read a model with ``iwfm.io``, modify it
(:func:`iwfm.io.create_scenario`), **run it**, then compare results
(:func:`iwfm.io.compare_models`).
...
batch files invoke them::
    ...
    # Or step by step:
    from iwfm import run_preprocessor, run_simulation
...
def _find_input_file(model_dir, step):
    from iwfm.io.model_adapter import _find_main_file
...
def run_step(step, model_dir, input_file=None, bin_dir=None,
    ...
    if os.name != "nt":
        raise OSError("The IWFM executables are Windows-only; "
                      "iwfm.io works on any OS, but running models does not.")
