...
### Find the function you need (`iwfm-io api`)

`iwfm-io` reads **and** writes every dataset in every IWFM input file, so
most things you might be about to implement already exist. Search the API
index by keyword instead of guessing:

```bash
iwfm-io api "water year"     # matching names, with signatures
iwfm-io api                  # the whole index, grouped by intent
```

```python
import iwfm_io
iwfm_io.find("water year")   # the same search, in process
```

The index is [`docs/API_INDEX.md`](docs/API_INDEX.md) — every public name
on one line — and ships inside the package, so it works from any install.

Writing code (or pointing an AI agent) at a project that handles IWFM
files? `iwfm-io init-agent` drops a short "call `iwfm-io`, don't
reimplement it" block into that project's `CLAUDE.md`, listing the
conventions hand-written IWFM parsers get wrong (the `24:00` date
convention, `Cc*` comment characters, load-bearing terminating comments,
simulation-anchored budget windows). Preview it with
`iwfm-io init-agent --print`.
...
- **v2.17.1** (2026-09-21) - **v4.1/v4.11 root-zone decks may omit the KPonded column (issue #37, second half).** 2.15.3 fixed the `ZRZBUDFL/` keyword tag this issue was opened for; underneath it sat an unrelated defect. IWFM sizes the root-zone soil table from its **first row**: it tries to read 14 reals out of that line and, when that fails, reads the whole table 13 columns wide (`RootZone_v411.f90`, "Backward compatibility: Check if the user entered KPonded values at all"; the identical block is in `RootZone_v41.f90`, unchanged between the 2015.3.1443 and 2025.0.1747 sources). A 13-column deck is therefore first-class v4.11 input rather than a legacy accident — the reporter's is a current production model that runs under IWFM 2015.3.1443. The reader required 14: strict mode raised at the first soil row, and lenient mode warned once per element (10,593 times on that deck) and returned `k_ponded` all-NaN, which propagated into every downstream calculation. The reader now mirrors the probe — a first row short of 14 values reads the table 13 wide and `k_ponded` is simply absent, as it is in the file, so the deck still writes back at 13 columns. Fixed with it: an absent KPonded means "same as K", exactly as the `-1.0` sentinel does in the 14-column form, and nothing told a caller that — `-1.0` was being read faithfully and then used as a conductivity. New **`RootZoneMain.k_ponded()`** resolves both forms into the values IWFM actually runs with, while `element_params` stays faithful to the file. Verified by stripping the 14th column from all 32,537 rows of C2VSimFG v1.5's real v4.11 deck: it reads in strict mode, column-for-column identical to the full file. A row that goes short *after* a 14-column first row still raises, since the width comes from row 1 as it does in IWFM. Also: `iwfm_io.models` is now covered by the API index (reader dataclasses were not in any indexed namespace, so their methods — including this one — could not be found with `iwfm-io api`), and `RootZoneMain` is exported from `iwfm_io.models`.
- **v2.17.0** (2026-09-19) - **Finding the function you need: `iwfm-io api`, a generated API index, and `iwfm-io init-agent`.** The package exports 179 top-level names plus the `plots`, `pest` and `dll` subpackages, and the only complete listing was a 790-line API reference that nothing pointed at — so code that already existed kept getting written a second time, most often in exactly the places where an IWFM convention is easy to get wrong (a `_24:00` stamp parsing to the next day's midnight while belonging to the named day, the `Cc*` comment characters with a leading `/` meaning a blank data value, simulation-anchored rather than calendar budget windows, terminating comments that IWFM *counts*). Three surfaces close that gap. **`iwfm-io api <keyword>`** searches every public name — 693 entries including class methods such as `IOModelAdapter.budget_df` — and prints the matches with their signature and docstring summary; `iwfm_io.find("water year")` runs the same search in process. **`iwfm_io/API_INDEX.md`** (mirrored as `docs/API_INDEX.md`) is that index in full, one line per name, grouped by intent; it is generated from the package itself by `python -m iwfm_io._api_index --write` and ships inside the wheel, so the search works from any install with no repository checkout, and serving it reads the file rather than importing matplotlib. **`iwfm-io init-agent [dir]`** writes a short "call iwfm-io, don't reimplement it" block into a project's `CLAUDE.md` (or `--file AGENTS.md`) between HTML markers — what never to hand-roll, and the convention behind each entry; re-running replaces the block in place and leaves the rest of the file untouched. Fixed alongside them: **`open_model` was missing from `iwfm_io.__all__`**, together with `create_scenario`, `compare_models`, `diff_model_files`, `head_difference`, `budget_difference`, `set_keyed_value` and `replace_text` — the package's documented entry point was invisible to `from iwfm_io import *` and to anything introspecting the namespace. `__all__` is now complete (179 names) and a test keeps it so, alongside checks for index drift, for a module added without an intent group, and for every rendered line being searchable back (a regex that silently made all return-annotated signatures unfindable was caught this way). The `iwfm-analyst` skill now triggers on code tasks as well as analysis sessions and leads with the same rule.
- **v2.16.0** (2026-09-19) - **Fixes for public issues #38, #39 and #40.** **#38 — `write_gw_main` dropped the comment that ends the output-file list.** IWFM decides whether the optional `IHTPFLAG` entry is present by counting the data lines of that block (`Class_AppGW::New`: 19 means no flag, anything else means it is there), so with no comment after `FNGWFL` the count ran on into `KDEB`, `NOUTH` and `FACTXY` and every entry was read one position out of step — IWFM 2024.2 gave up at the `GWHYDOUTFL` line of a round-tripped C2VSimFG v1.5 GW main. The writer now terminates the block. The other two line-count gates in the 2025.0 source were checked too: the root-zone main's file list is already terminated (now covered by a test), and the simulation main's solution-scheme block is counted to EOF, so no terminator applies. **#39 — NAME fields were not cut at a glued `/`.** IWFM reads name-bearing rows with `StripTextUntilCharacter(ALine,'/')`, which cuts at the first `/` whatever precedes it, while the readers' separator needed whitespace (or a digit) in front. A name like `N/A` — diversion 498 of C2VSimFG v1.5 — survived as `N/A` where the model reads `N`, and because the writers refuse a `/` inside a NAME (it would be truncated on the next read), a plain read → write of an untouched deck raised. A shared `split_name_notes()` now cuts every positional NAME at the first `/` and keeps the remainder as the row's annotation: diversion and bypass specs, stream/GW/BC/subsidence/tile-drain hydrographs, lake names, stream reach names and subregion names. Annotations themselves may now contain `/` (they sit after the row's separator, where IWFM has already stopped reading), so `fmt_note` replaces `fmt_name` for the cells that are written as `/ note`. **#40 — time-series objects answer both spec shapes.** Seven classes held a `TimeSeriesSpec` in `.spec`; five carried the five parameters flat (they also need `has_dssfl`/`keywords`). Generic "rewrite any time-series file" code had to branch, and failed late — `AttributeError: 'TimeSeriesDataFile' object has no attribute 'spec'` on the stream surface-area file. The flat classes gained a `.spec` property with a setter that writes the fields back, and the spec-shaped ones gained flat pass-throughs, so both idioms work wherever the parameter exists; `.spec` is the shape every class answers in full (a file format with no `FACT` reports the spec's neutral 1.0). `TimeSeriesSpec`, `TimeSeriesDataFile`, `TimeSeriesFile`, `FileHeader`, `ConversionFactor` and `ZoneDefinition` are now exported from `iwfm_io` and `iwfm_io.models` instead of needing `iwfm_io.models.base`.
- **v2.15.3** (2026-09-16) - **Fixes for public issues #36 and #37, both regressions from 2.14.0.** **#36 — zone reads refused per-zone columns.** `IWFMZBudget.get_values_for_zone` and `get_values_for_zones_interval` take *diversified* column indices: the general column list has one lumped "Inflow from / Outflow to adjacent zones" pair, and for a zone with *k* neighbours that pair becomes one pair per neighbour, so the zone has `len(general) - 2 + 2k` columns. 2.14.0 checked against the general count and refused every column past it for any zone with more than one neighbour. Both reads now check against the zone's exact diversified count (from `get_column_headers_for_zone`, cached per zone list); exact matters because the DLL allocates the zone's flow array to that size and indexes it unchecked. `get_values_for_zones_interval` also accepts 0-padding again for zones with shorter column lists (IWFM stops a zone's list at the first index below 2). And `get_column_headers_for_zone`'s default column list, 1..500, was itself an out-of-bounds read of the general headers — the "hang in 2025.0.1747" this project had recorded as a DLL bug; it now defaults to every general column and validates a given list. Verified on the sample model with builds 2025.0.1747 and 2015.3.1443 and on C2VSimFG v1.5 (build 2024.2.1594): every diversified column reads, and each zone's "Inflow from zone B" equals zone B's "Outflow to zone A". **#37 — keyword tags written `/ KEYWORD/`.** IWFM stops reading a keyed line at the first `/`, so a tag with a trailing slash is valid, but the readers dispatched on the raw first word (`ZRZBUDFL/`), which matched nothing and ended the keyword block early. In the root-zone main that handed the line to the soil-parameter table, which 2.14 correctly refused — and which 2.12.0 had silently lost entirely, along with `FACTK`, `FACTCPRISE`, `TUNITK` and the paths after the tag. A new `keyword_name()` helper normalises the tag at all 16 keyword-dispatch sites (root-zone, groundwater, stream, simulation, time-series and parameter-block readers). `read_rootzone_main` also takes `n_elements=` and reads exactly that many soil rows, as IWFM does (`open_model` passes it); without a count, a keyed line ends the table instead of raising.
